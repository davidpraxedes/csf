...
export default function CPFConsultPage() {
  ...
  const handleConsultar = async () => {
    ...
    try {
      ...
      setError(err.message || 'Não foi possível consultar os dados. Você pode continuar mesmo assim.');
      setCPF(cpfLimpo);
    } finally {
      setLoading(false);
    }
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 md:p-10">
                ...
                {error && (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-amber-700 text-sm">Você pode continuar mesmo assim.</p>
                    </div>
                  </motion.div>
                )}
            ...
            {error && !dadosEncontrados && !loading && (
              <div className="mt-4">
                <button
                  onClick={handleContinuar}
                  className="w-full border-2 border-carrefour-blue text-carrefour-blue hover:bg-carrefour-blue hover:text-white font-semibold text-lg py-4 px-6 rounded-xl transition-all duration-300"
                >
